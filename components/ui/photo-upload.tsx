...
export function PhotoUpload({ ticketId, attachmentToken }: { ticketId: string; attachmentToken?: string }) {
  ...
  return (
    <div className="mt-4">
      ...
      <label className="inline-flex items-center gap-2 border border-ink-950/15 px-3 py-2 text-xs font-medium text-ink-950 cursor-pointer hover:border-ink-950/30">
      ...
      <p className="mt-1 text-xs text-ink-800/50">JPEG, PNG, or WebP — up to 5MB</p>
    </div>
  );
}
