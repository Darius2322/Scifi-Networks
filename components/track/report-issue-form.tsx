  ...
  if (ticketNumber) {
    return (
      <div className="animate-success card border-status-good/30 bg-status-good/5 p-6">
        <p className="text-sm font-semibold text-status-good">Report received</p>
        <p className="mt-2 font-display text-xl font-bold text-ink-950">{ticketNumber}</p>
        <p className="mt-2 text-sm text-ink-800">We'll update this in your dashboard as it progresses.</p>
  ...
  return (
    <form onSubmit={handleSubmit} className="card space-y-5 p-6" noValidate>
      ...
      <div>
        ...
        <select id="issue_option" name="issue_option" required className="field mt-1.5">
          {ISSUE_TYPES.map((opt) => (
      ...
      <div>
        ...
        <input id="location_text" name="location_text" className="field mt-1.5" />
      </div>
