...
export async function GET(_req: Request, { params }: { params: { id: string } }) {
  ...
  const performerIds = [...new Set((data ?? []).map((t: any) => t.performed_by).filter(Boolean))] as string[];
  ...
  return NextResponse.json({
    transactions: (data ?? []).map((t: any) => ({ ...t, performed_by_name: names[t.performed_by] ?? 'Unknown' })),
  });
}
