      ...
      <div className="grid lg:grid-cols-[1fr_320px] gap-8">
        <div className="space-y-8">
          <section className="card p-6">
            <p className="text-sm text-ink-700">Ticket</p>
          ...
          <section className="card p-6">
            <h2 className="font-semibold text-ink-950">Request status</h2>
            <div className="mt-5">
          ...
          <section className="card p-6">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold text-ink-950">Your support tickets</h2>
            </div>
            {tickets.length === 0 ? (
              <p className="mt-3 text-sm text-ink-700">No support tickets yet.</p>
            ) : (
                ...
                {tickets.map((t) => (
                  <li key={t.id} className="py-3 flex items-center justify-between text-sm">
                    <div>
                      ...
                      <p className="text-ink-700">{t.ticket_number}</p>
                    </div>
                    <span className="text-xs uppercase tracking-wide text-ink-700">{t.status.replace('_', ' ')}</span>
                  </li>
                ))}
              </ul>
            )}
        ...
        <aside className="space-y-6">
          <div className="card p-5">
            ...
            {notifications.length === 0 ? (
              <p className="mt-2 text-sm text-ink-700">Nothing new yet.</p>
            ) : (
